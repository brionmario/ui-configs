# Contributing to Brion Mario's UI Configs

I would love for you to contribute to my UI Configs and help make it even better than it is today!
...
```shell
git clone https://github.com/<github username>/ui-configs.git
```

3. Set the original repo as the upstream remote.

```shell
git remote add upstream https://github.com/brionmario/ui-configs.git
```
...
* `eslint` - Changes to the `@bionmario/eslint-plugin` package.
* `prettier` - Changes to the `@bionmario/prettier-config` package.
* `stylelint` - Changes to the `@bionmario/stylelint-config` package.
